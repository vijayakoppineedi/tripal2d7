...
if (count($object_rels) > 0 or count($subject_rels) > 0) { ?>
  ...
  foreach ($subject_rels as $rel_type => $rels){
	...
	$genomic_link = "<a href='#' onclick=\"popup_message_display_popup(".$object->record->object_id->feature_id.", '".$object->record->object_id->type_id->name."', 680, 300);\">Geno Link</a>";  
	...
	// cDNA sequence
        ...
        if(!empty($cdna_sequence->residues))
          $cDNA_link = "<a href='#' onclick=\"popup_message_display_popup(".$object->record->subject_id->feature_id.", '".$object->record->subject_id->type_id->name."', 680, 300);\">cDNA Link</a>";

	// CDS sequence			
	$type_id_cds = CDS_TYPE_ID; //'325';
	$cds_args = array(':feature_id' => $object->record->subject_id->feature_id, ':type_id_cds' => $type_id_cds); 
        $cds_sql = "select fr.subject_id, f.residues, c.name from  chado.feature_relationship fr, chado.feature f, chado.featureloc fc, chado.cvterm  c where f.feature_id=fr.subject_id and fc.feature_id=f.feature_id and f.type_id=c.cvterm_id and fr.object_id=:feature_id and c.cvterm_id=:type_id_cds";	
	$cds_sequence = chado_query($cds_sql, $cds_args)->fetchObject();		

        $cds_link = '-';
        if(!empty($cds_sequence->residues))
          $cds_link = "<a href='#' onclick=\"popup_message_display_popup(".$cds_sequence->subject_id.", '".$cds_sequence->name."', 680, 300);\">CDS Link</a>";
	...
	$type_id_pep = PEP_TYPE_ID; //324;
	  ...
	  $pep_link = "<a href='#' onclick=\"popup_message_display_popup(".$result->subject_id.", '".$result->typename."', 680, 300);\">Pep Link</a>"; 		
        } 		
  ...
  foreach ($object_rels as $rel_type => $rels){
	...
	$obj_feature = chado_expand_var($subject, 'table', 'featureloc');  		
        ...
        // Genomic sequence - nothing but co-ordinates sequence 		 		  
  	$genomic_link = "<a href='#' onclick=\"popup_message_display_popup(".$subject->record->object_id->feature_id.", '".$subject->record->object_id->type_id->name."', 680, 300);\">Genomic Fasta</a>";  
	...
	$cDNA_link = "-";
        ...
        if(!empty($cdna_sequence->residues))	
	  $cDNA_link = "<a href='#' onclick=\"popup_message_display_popup(".$subject->record->subject_id->feature_id.", '".$subject->record->subject_id->type_id->name."', 680, 300);\">cDNA Fasta</a>"; 
